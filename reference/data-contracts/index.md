...
`SourceryEngine.extract_stream(...)` runs up to `batch_concurrency` chunks concurrently and emits each finished batch in deterministic chunk order. Events report added extractions, completed chunks, and completed passes; the final `ExtractResult` is returned when the generator is exhausted.
